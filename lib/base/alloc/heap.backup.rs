...
///
/// This is actually a header that we store at the start of the block.
///
/// We don't store any size information in the header,
/// because we a separate free block array for each block size.
...
impl<'a> Heap<'a> {
   ...
   pub unsafe fn buddy(&self, order: usize, block: *mut u8) -> Option<*mut u8> {
      ...
      return if size >= self.heapSize {
         // The main heap itself does not have a buddy.
         None
      } else {
